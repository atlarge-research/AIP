...
from PyQt5.QtWidgets import QApplication, QWidget, QTableWidget, \
    ...
    QLineEdit, QGroupBox, QFormLayout, QLabel, QRadioButton, QButtonGroup, \
...
class App(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.table.setHorizontalHeaderLabels(
            ["Title", "Abstract", "Venue", "Year", "DOI"])
        self.table.setWordWrap(True)

        # Make the table span the entire width. This code must be after
        # setting row and columns or it won't do anything.
        view_types = [QHeaderView.Stretch, QHeaderView.Stretch,
        ...
        for column in range(horizontal_header.count()):
            ...
            horizontal_header.setSectionResizeMode(column,
    ...
    def createFormGroupBox(self):
        ...
        self.keyword_filter_options = [QRadioButton("All"),
                                       QRadioButton("Any")]
        ...
        for i in range(len(self.keyword_filter_options)):
            ...
            self.keyword_option_button_group.addButton(
                self.keyword_filter_options[i], i)

        filter_label = QLabel("Match keywords:")
        filter_label.setToolTip(
            '''All will find articles containing ALL keywords,
            whereas any will match articles containing ANY of the keywords''')
        layout.addRow(filter_label, button_layout)

        self.blackwords_input = QLineEdit()
        self.blackwords_input.setReadOnly(False)
        self.blackwords_input.setPlaceholderText(
            "Comma separated filter terms")
        layout.addRow(QLabel("Exclusion filter:"), self.blackwords_input)

        self.blackword_filter_options = [QRadioButton("All"),
                                         QRadioButton("Any")]
        ...
        for i in range(len(self.blackword_filter_options)):
            ...
            # Add each radio button to the button group & give it an ID of i
            self.blackwords_option_button_group.addButton(
                self.blackword_filter_options[i], i)

        filter_label = QLabel("Filter option:")
        filter_label.setToolTip(
            '''All will remove articles containing ALL keywords,
             whereas any will remove articles containing ANY keywords''')
        layout.addRow(filter_label, button_layout)

        self.venues_input = QLineEdit()
        self.venues_input.setReadOnly(False)
        self.venues_input.setPlaceholderText(
            "Comma separated venue acronyms, leave blank for all")
        layout.addRow(QLabel("Venues:"), self.venues_input)

        only_int = QIntValidator()
        only_int.setRange(1900, 2100)
        ...
        self.start_year_input.setValidator(only_int)
        ...
        self.end_year_input.setValidator(only_int)

        year_range_layout.addWidget(QLabel("End year:"))
        year_range_layout.addWidget(self.end_year_input)

        year_range_layout.addStretch()  # Fill space to prevent it centering
        # elements

        layout.addRow(QLabel("Start year:"),
                      year_range_layout)  # Don't add the Qlabel to the
        # QHBoxLayout, it messes
        ...
        for i in range(len(self.matching_articles)):
            for j in range(5):
                self.table.setItem(i, j, QTableWidgetItem(
                    str(self.matching_articles[i][j])))

        self.search_button.setDisabled(False)

    def get_keywords(self):
        return [str(k).lower().strip() for k in
                self.keyword_input.text().split(",") if len(k) > 0]

    def get_blackwords(self):
        return [str(k).lower().strip() for k in
                self.blackwords_input.text().split(",") if len(k) > 0]

    def get_venues(self):
        if len(self.venues_input.text().strip()) > 0:
            return [str(k).lower().strip() for k in
                    self.venues_input.text().split(",")]
        else:
    ...
    def search(self):
        ...
        blackword_filter = all if self.blackword_filter_options[
            0].isChecked() else any
        ...
        for keyword in keywords:
            keyword = keyword.lower()  # Use lower case so that we do not
            # potentially miss articles
            if match_all_keywords and first_run_done and len(
                    self.matching_articles) == 0:
                break  # If we already are out of matches, just skip the
                # other keywords.
        ...
        for group in grouped_keywords:
            query_parts_per_group_of_aliases.append("({})".format(
                " OR ".join(
                    ["(lower(title) LIKE ? OR lower(abstract) LIKE ?)"] * len(
                        group))
            ))

            # Add all aliases twice, once to match in the title and one to
            # match for in the abstract
            for i in group:
                keywords_for_query.append(i)
                keywords_for_query.append(i)

        query_operator = " AND " if match_all_keywords else " OR "
        query = '''SELECT title, abstract, venue, year,
        doi, id FROM publications WHERE LOWER(venue) IN
         ({0}) AND {1} AND year BETWEEN ? AND ? ORDER BY year DESC'''.format(
            ",".join(["?"] * len(venues)),
        ...
        query_result = c.execute(query,
                                 venues + keywords_for_query + [start_year,
                                                                end_year])
        ...
        for row in query_result.fetchall():
            ...
            if len(blackword_set) > 0 \
                    and (
                    blackword_filter(
                        word in title.lower() for word in blackword_set) or
                    blackword_filter(word in abstract.lower() for word in
                                     blackword_set)):
                continue
