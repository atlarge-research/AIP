...
from .logic import calculate_authors_citations, get_avg_and_std_per_band, \
...
def rising_stars(request):
    ...
    avg_per_band, std_per_band = get_avg_and_std_per_band(request, citations,
                                                          authors)
    number = int(request.query_params.get('number'))
    z_scores = get_z_scores(request, avg_per_band, std_per_band, citations,
                            authors)
...
def rising_stars_page_rank(request):
    ...
    avg_per_band, std_per_band = get_avg_and_std_per_band_page_rank(request,
                                                                    page_ranks,
                                                                    authors)
    z_scores = get_z_scores_page_rank(request, avg_per_band, std_per_band,
                                      page_ranks, authors)
    number = int(request.query_params.get('number'))
    number = min(number, len(z_scores))
    result = z_scores[0:number]
...
def rising_stars_clusters(request):
    ...
    avg_per_band, std_per_band = \
        get_avg_and_std_per_band_communities(request,
                                             citations, communities,
                                             authors)
    z_scores = get_z_scores_communities(request, avg_per_band, std_per_band,
...
def local_rising_stars(request):
    ...
    scores = calculate_scores(citations, keywords, author_keywords)
    print(scores)
    avg_per_band, std_per_band = get_avg_and_std_per_band(request, scores,
                                                          authors)
    z_scores = get_z_scores(request, avg_per_band, std_per_band, scores,
                            authors)
    number = int(request.query_params.get('number'))
    number = min(number, len(z_scores))
    result = z_scores[0:number]
