...
def remove_authors_field(json_dictionary):
    ...
    filtered_dict = {
        key: value for key, value in json_dictionary.items()
        if key != "authors"
    }
...
class PublicationsTests(APITestCase):
    ...
    def test_filtering_publications1(self):
        ...
        self.assertCountEqual(
            modify_dictionary_list(json.loads(response.content)),
            modify_dictionary_list(
                list(
                    filter(
                        lambda d: d["title"] == "World",
                        self.all_publications,
                    ),
                ),
            ),
    ...
    def test_filtering_publications4(self):
        ...
        self.assertCountEqual(
            ...
            modify_dictionary_list(
                list(
                filter(
                    lambda d: 2012 > d["year"] > 2000 and d[
                    "venue"
                    ] == "TUD" and d["n_citations"] == 178,
                       self.all_publications,
                ),
                ),
            ),
    ...
    def test_filtering_abstract1(self):
        ...
        self.assertCountEqual(
            ...
            modify_dictionary_list(
                list(
                filter(
                    lambda d: "computer" in d["abstract"],
                    self.all_publications,
                ),
                ),
            ),
    ...
    def test_filtering_abstract2(self):
        ...
        self.assertCountEqual(
            ...
            modify_dictionary_list(
                list(
                filter(
                    lambda d: "computer" in d["abstract"] and
                              "more" in d["abstract"],
                    self.all_publications,
                ),
                ),
        ...
        self.assertCountEqual(
            ...
            map(
                ...
                modify_dictionary_list(
                    list(
                    filter(
                        lambda d: d["year"] > 2000 and d["venue"] == "TUD",
                        self.all_publications,
                    ),
                    ),
                ),
