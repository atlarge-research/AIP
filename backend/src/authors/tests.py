...
class AuthorsTests(APITestCase):
    ...
    def test_filtering_authors1(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertCountEqual(
            json.loads(response.content), list(
            filter(
    ...
    def test_filtering_authors2(self):
        ...
        response = self.client.get(
            url + "?first_publication_year__gt=2000&"
    ...
    def test_filtering_authors3(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            json.loads(response.content).get("results"),
            [{
                "name": "William",
...
class RisingStarsTest(APITestCase):
    ...
    def test_rs_identification_clusters(self):
        first_two_authors = self.send_request_helper(
            self.rs_clusters_url,
            2016,
        )
