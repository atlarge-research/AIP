...
def get_avg_and_std_per_band(
    ...
    for author in authors:
        if (
            author.first_publication_year is not None
...
def get_avg_and_std_per_band_page_rank(
    request, page_ranks,
    authors=Authors.objects.all(),
):
    """
...
def get_z_scores_page_rank(
    request, avg_per_band, std_per_band, page_ranks,
    authors=Authors.objects.all(),
):
    """
...
def get_avg_and_std_per_band_communities(
    request, citations, communities,
    authors=Authors.objects.all(),
):
    """
