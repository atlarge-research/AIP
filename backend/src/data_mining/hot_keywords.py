...
from .raw_db_access import (
    get_all_citations_year_range, get_paper_years, get_pub_citations_years,
...
def filter_publications(kp_dicts, avg_publications, p_max):
    ...
    filtered_dict = {
        k: v for k, v in kp_dicts.items() if
...
def filter_growth(kp_dict, p1, p2, r_min):
    ...
    filtered_dict = {
        k: v for k, v in kp_dict.items() if p2[k] != 0 and
        p1[k] / p2[k] >= r_min
    }
    return filtered_dict
...
def calculate_p_max(year, p_rate):
    ...
    pub_in_year = (
        pub_per_years[year] + pub_per_years[year - 1] +
        pub_per_years[year - 2]
    ) / 3
...
def run_filters(kp_dict, dt, year, pub_avg, r_min, p_max, c_min):
    ...
    kp_dict_filtered2 = filter_citations(
        kp_dict_filtered1, dt, year,
        c_min,
    )
...
def filter_keywords(
    kp_dict, dt=5, year=2020, r_min=5, p_max=-1, c_min=-1,
    ...
    if r_adjustment:
        ...
        while len(final_dict.keys()) < feature_no and r_min > 1:
            r_min -= 0.5
            final_dict = run_filters(
                kp_dict, dt, year, pub_avg, r_min, p_max,
                c_min,
            )
