...
def complement_with_aminer(aminer_root):
    ...
    # part 10 was corrupt at one point and could not be downloaded and was not in the corpus_location folderT
    for file in glob.iglob(os.path.join(aminer_root, "*.txt")):
        aminer_file = os.path.join(aminer_root, file)  # Get the path of the file
        with open(aminer_file, "r", encoding='ISO-8859-1') as json_file:
            print(aminer_file)
            ...
            for publication in publication_iterator:
                ...
                if 'venue' not in publication:
                    logger.warning("Skipping line missing venue: %s in %s.", publication, aminer_file)
                    continue

                if 'title' not in publication:
                    logger.warning("Skipping line missing title: %s in %s.", publication, aminer_file)
                    continue
