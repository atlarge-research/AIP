...
top_keywords_for_query = get_top_keywords_for_query(
    conn, corpus_df,
...
top_keywords_for_query = get_top_keywords_for_query(
    ...
    FROM publications 
    WHERE year between {} and {} 
    and (lower(title) like '%workflow%'
    or lower(abstract) like '%workflow%') 
    and (lower(title) like '%schedul%'
    or lower(abstract) like '%schedul%')""".format(
        start_year, end_year),
    40)
...
print("""\\begin{{table}}[t]
\\caption{{Top-10 keywords in scheduling workflow articles
in the past decade.}}
...
for year in range(start_year, end_year + 1):
    query = '''SELECT * FROM publications WHERE year = ? and (lower(title)
    like '%workflow%' or lower(abstract) like '%workflow%') and
    (lower(title) like '%schedul%' or lower(abstract) like '%schedul%')'''
    keywords = get_top_keywords_for_query(conn, corpus_df, query, num_keywords,
                                          [year])
    keywords_per_year[year] = keywords

print("""
\\begin{table}[t]
\\caption{Top-10 keywords in scheduling workflow articles in the past
 decade per year.}
